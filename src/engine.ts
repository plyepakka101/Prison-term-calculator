...
export function calculateCase(input:CalculationInput){
 ...
 const elapsedConfinementDays=hasConfinement?inclusiveDays(confinementStart,checkDate):0;
 ...
 // วันปล่อยตัว = วันถัดไปหลังวันสุดท้ายของการกักขัง
 const confinementRelease = confinementEnd ? new Date(confinementEnd.getTime() + DAY_MS) : null;
 // สถานะชำระ: ชำระครบเมื่อ fineRemaining === 0
 const isFinePaid = typeof fineRemaining !== 'undefined' ? fineRemaining === 0 : false;
 const status = isFinePaid ? 'PAID' : 'UNPAID';
 return {mode:input.mode,status,sentence:{...input.sentence,nominalAllocationDays:sentenceDaysForAllocation,pretrialDays,imprisonmentCreditDays,remainingPretrialForFine,statutoryEnd:statutoryImprisonmentEnd,projectedRelease:imprisonmentRelease},fine:{initialRemaining:initialFineRemaining,pretrialFineCredit,confinementCredit:elapsedConfinementCredit,paidToday,fineBeforeToday,remaining:fineRemaining},confinement:{remainingDays:remainingConfinementDays,maximumDays:(input.maxConfinementYears||1)*365,projectedEnd:confinementRelease}};
}
